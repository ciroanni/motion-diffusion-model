...
def main():
    # add input by user with argparse
    import argparse
    parser = argparse.ArgumentParser(description='Process motion data from npy file.')
    parser.add_argument('--prompt', type=str, required=True, help='Prompt to generate motion')
    parser.add_argument('--num_repetitions', type=int, required=True, help='Number of repetitions')
    ...
    predictor = Predictor()
    predictor.setup()
    output = predictor.predict(args.prompt, args.num_repetitions, args.output_format)
    print(output)